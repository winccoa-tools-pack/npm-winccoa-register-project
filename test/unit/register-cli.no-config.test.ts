...
function runCli(args: string[]) {
  return spawnSync('node', [path.join(process.cwd(), 'dist', 'src', 'index.js'), ...args], { encoding: 'utf8', env: process.env })
}

function testNoConfigWhenNotRunnable() {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'regtest-'))
  const projectPath = tmp
  const res = runCli(['--project-path', projectPath, '--runnable', 'false'])
